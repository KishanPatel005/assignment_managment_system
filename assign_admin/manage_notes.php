...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $semm = "Your notes current sem is " . $_GET['sem'];

    $subjectss = $_GET['subject'];
    $sell = mysqli_query($con,"SELECT * FROM notes WHERE id = '$id'");
...
if (isset($_POST['submit'])) {
    ...
    $checkSubject = mysqli_query($con, "SELECT id FROM notes WHERE sem_id = '$sem' AND subjects_id = '$subject' AND title = '$title' AND notes = '$notes_link'");
    if (mysqli_num_rows($checkSubject) > 0) {
        ...
        if (isset($_GET['id'])) {
            ...
            $update = mysqli_query($con, "UPDATE `notes` SET `sem_id`='$sem',`subjects_id`='$subject',`title`='$title',`notes`='$notes_link' WHERE id = '$id'");
        } else {
            $insert = mysqli_query($con, "INSERT INTO `notes`(`sem_id`, `subjects_id`,`title`,`notes`) VALUES ('$sem','$subject','$title','$notes_link')");
        }try {
       ...
       <body>
           <table border="5">   
               <tr>
                   <td>NEW NOTE ADDED   in    http://ait.lovestoblog.com/</td></tr><tr>
            
                   <td>TITLE = '.$title.'</td></tr><tr>
                   <td>NOTE LINK = '.$notes_link.'</td></tr>
               </tr>
           </table>
       </body>
       </html>';
                    ...
                    <div class="card">
                        ...
                        <div class="card-body card-block">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="title" id="company" placeholder="Enter your notes title" class="form-control" value="<?php echo $title; ?>" required>
                            </div>



                            <div class="form-group">
                                <label for="company" class="form-control-label">notes link</label>
                                <input type="text" name="notes_link" id="company" placeholder="Enter your notes link" class="form-control" value="<?php echo $link; ?>" required>
                            </div>
